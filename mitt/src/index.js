...
class CooknjsEggsMittCommand extends Command {
  async run() {
    const { flags } = this.parse(CooknjsEggsMittCommand)

    if (flags.run) {
      this.runServer()
    }

    if (!flags.run) console.log(`I'm Mitt, a static server built on Fastify for Cook'n'JS`)
  }
...
CooknjsEggsMittCommand.description = `Mitt is a simple static server, built on Fastify, for Cook'n'JS.
...
Extra documentation goes here
`

CooknjsEggsMittCommand.flags = {
  // add --version flag to show CLI version
  version: flags.version({ char: 'v' }),
  // add --help flag to show CLI version
  help: flags.help({ char: 'h' }),
  run: flags.boolean({ char: 'r', description: 'Start the server' }),
}

module.exports = CooknjsEggsMittCommand
